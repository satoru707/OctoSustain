...
export function useRealTimeDashboard(podId: string, token?: string) {
  ...
  const [tentacleAnimations, setTentacleAnimations] = useState<Record<string, number>>({})
  ...
  useEffect(() => {
    ...
    const handleMembersUpdate = (data: { onlineMembers: MemberActivity[]; totalOnline: number }) => {
      setOnlineMembers(data.onlineMembers)
    }

    const handleMemberJoined = (data: { userId: string; username: string; timestamp: Date }) => {
    ...
    const handleMemberLeft = (data: { userId: string; username: string; timestamp: Date }) => {
    ...
    const handleTentacleAnimation = (data: { category: string; progress: number }) => {
