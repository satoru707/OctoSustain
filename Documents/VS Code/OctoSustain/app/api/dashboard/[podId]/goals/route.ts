...
export async function GET(request: NextRequest, { params }: { params: { podId: string } }) {
  try {
    ...
    const goals = {
      ...
      transport: { target: 70, current: 45, unit: "km", deadline: "2024-12-31" },
      water: { target: 100, current: 85, unit: "L", deadline: "2024-12-31" },
      food: { target: 90, current: 55, unit: "meals", deadline: "2024-12-31" },
      custom: { target: 50, current: 30, unit: "actions", deadline: "2024-12-31" },
    }
...
export async function PUT(request: NextRequest, { params }: { params: { podId: string } }) {
  try {
    const token = request.cookies.get("token")?.value
    if (!token) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
