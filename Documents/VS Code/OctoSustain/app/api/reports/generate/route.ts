...
export async function POST(request: NextRequest) {
  try {
    ...
    const { podId, type, period, startDate, endDate, includeCharts, includeDetails, format } = body
    ...
    const totalCO2Saved = tentacleLogs.reduce((sum, log) => sum + log.co2Saved, 0)
    const totalPoints = tentacleLogs.reduce((sum, log) => sum + log.points, 0)
    const participantCount = new Set(tentacleLogs.map((log) => log.userId)).size

    // Group by category
    const categories = tentacleLogs.reduce(
      (acc, log) => {
        if (!acc[log.category]) {
    ...
    const reportData = {
      summary: {
        ...
        totalPoints,
        participantCount,
        activeDays: Math.ceil((reportEndDate.getTime() - reportStartDate.getTime()) / (1000 * 60 * 60 * 24)),
      ...
      categories,
      achievements: [
        totalCO2Saved > 100 ? "Exceeded 100kg CO2 reduction!" : "Making great progress on CO2 reduction",
        participantCount > 5 ? "Strong pod participation" : "Growing pod engagement",
        Object.keys(categories).length > 3 ? "Multi-category impact" : "Focused environmental impact",
      ],
      previewData: {
        chartData,
