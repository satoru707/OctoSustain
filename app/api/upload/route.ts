...
export async function POST(request: NextRequest) {
  try {
    ...
    const rules = fileValidationRules[category as keyof typeof fileValidationRules] || fileValidationRules.general

    if (filesToProcess.length > rules.maxFiles) {
      return NextResponse.json({ error: `Maximum ${rules.maxFiles} files allowed` }, { status: 400 })
    }
    ...
    for (const file of filesToProcess) {
      ...
      if (!rules.allowedTypes.includes(file.type)) {
        return NextResponse.json(
          {
            error: `Invalid file type: ${file.type}. Allowed: ${rules.allowedTypes.join(", ")}`,
          },
          { status: 400 },
        )
      }
      ...
      if (file.size > rules.maxSize) {
        return NextResponse.json(
          {
            error: `File too large: ${file.name}. Max size: ${rules.maxSize / 1024 / 1024}MB`,
          },
          { status: 400 },
        )
      }
