  ...
  try {
    ...
    const transformedChallenges = challenges.map((challenge) => {
      ...
      return {
        ...
        difficulty: challenge.points > 100 ? "Hard" : challenge.points > 50 ? "Medium" : "Easy",
        ...
        isJoined,
        requirements: ["Log daily progress", "Upload verification photos", "Complete daily check-ins"],
    ...
    // Apply filters
    if (filter === "active") {
      filteredChallenges = transformedChallenges.filter((c) => c.isJoined && new Date() < c.endDate)
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        active: transformedChallenges.filter((c) => c.isJoined && new Date() < c.endDate).length,
        completed: transformedChallenges.filter((c) => c.isJoined && new Date() > c.endDate).length,
        available: transformedChallenges.filter((c) => !c.isJoined && new Date() < c.endDate).length,
      },
    })
  } catch (error) {
