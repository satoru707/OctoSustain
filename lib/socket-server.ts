...
export function initializeSocketServer(server: NetServer): ServerIO {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-pod", (podId: string) => {
      ...
      const podSockets = Array.from(
        io.sockets.adapter.rooms.get(`pod:${podId}`) || []
      );
      ...
      socket.emit("pod-members-update", {
        onlineMembers,
    ...
    socket.on(
      ...
      (data: {
        ...
        console.log(
          `Tentacle update from ${authenticatedSocket.userId} in pod ${podId}:`,
          data
        );
      }
    );
    ...
    socket.on(
      "challenge-progress",
      (data: { challengeId: string; progress: number; podId: string }) => {
        ...
        socket
          ...
          .emit("challenge-leaderboard-update", {
            challengeId,
            userId: authenticatedSocket.userId,
            progress,
          });
        ...
        console.log(
          `Challenge progress from ${authenticatedSocket.userId}:`,
          data
        );
      }
    );
    ...
    socket.on("join-challenge", (challengeId: string) => {
      ...
      console.log(
        `User ${authenticatedSocket.userId} joined challenge ${challengeId}`
      );
    });
    ...
    socket.on(
      ...
      (data: {
        ...
        console.log(
          `New activity from ${authenticatedSocket.userId}:`,
          activity
        );
      }
    );
    ...
    socket.on("typing-start", (data: { podId: string; context: string }) => {
    ...
    socket.on("typing-stop", (data: { podId: string; context: string }) => {
      const { podId, context } = data;
    ...
    socket.on(
      ...
      (data: {
        ...
        console.log(
          `Notification sent from ${authenticatedSocket.userId} to ${targetUserId}`
        );
      }
    );
    ...
    socket.on("disconnect", () => {
      ...
      if (authenticatedSocket.podId) {
        socket
          .to(`pod:${authenticatedSocket.podId}`)
          .emit("member-disconnected", {
            userId: authenticatedSocket.userId,
            username: authenticatedSocket.username,
            timestamp: new Date(),
          });
